...
To add more vehicles, follow the shape documented in `data/SCHEMA.md` and append entries to the `models` array in `data/evs.json`, then run `npm run sync-urls` to fill in each new entry's `url` (the canonical page for that car, derived from its slug — don't hand-write it). The prerender step warns if you forget; the deployed dataset recomputes these regardless, so only the GitHub copy of the file is affected.
