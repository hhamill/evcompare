...
# TODO: epaSizeClass gap — 14 → 7 (2026-09-01)

`epaSizeClass` is generated by `npm run fetch-epa` from the id in `links.epaWindowSticker`, so a
record without a usable id is unclassified *and* uncovered by the audit's model-year and EPA-range
checks. 14 records were in that state.

## A second bad-link class, and a tightened check

Five of the 14 *did* have an `epaWindowSticker` — pointing at a fueleconomy.gov **PowerSearch
listing** rather than a vehicle entry. Right site, wrong granularity: `fetch-epa` reads the id out
of the URL with `/[?&]id=(\d+)/`, so a search page leaves the record exactly as underivable as a
wrong domain does, while looking correct in the data. Audit check 10 now requires both a
fueleconomy.gov host **and** an `?id=`. (This is the second distinct defect in one field today;
the first was 5 records pointing at manufacturer/review sites.)

## What actually fixed things: size class is a property of the vehicle

Only two records had an EPA entry matching them outright. The rest were unlocked by the principle
the EX60 record already established — **size class belongs to the vehicle, not to its trim, wheel,
charger or model year** — so a sibling entry settles it even when nothing describes the exact
record. Filled manually with the sourcing spelled out in `notes`, deliberately *without* adding an
`epaWindowSticker`, since the link would then claim to describe a car it doesn't:

| record | class | from |
| --- | --- | --- |
| Rivian R1S Dual-Motor Large | Standard SUV | **id 48746** — direct match |
| Rivian R1T Adventure Dual-Motor Large | Standard Pickup | **id 48756** — direct match |
| Rivian R1S Quad-Motor Max | Standard SUV | R1S Tri Max (48751); EPA certifies no Quad |
| Cadillac Optiq Luxury / Sport AWD | Standard SUV | 2026 OPTIQ AWD (49952); EPA has no 2025 Optiq |
| Chevrolet Silverado EV RST Max Range | Standard Pickup | Max Range WT (49639); EPA certifies Max Range only in WT |
| Tesla Cybertruck Cyberbeast | Standard Pickup | Cybertruck AWD (50039); EPA has never certified a Cyberbeast |

Worth noting the Optiq comes back **Standard SUV**, not Small — which changes the size-hub
candidate counts recorded earlier.

## The Rivian wheel question, now sourced but not settled

EPA splits the R1S/R1T Dual-Motor Large Pack by wheel: **20-inch = 300mi, 22-inch = 329mi**. Our
records say 329, so they describe the **22-inch** car, and they are now cited to the ids that
actually match (48746 / 48756) instead of a search page.

- [ ] **Open:** the base-configuration rule says a record should describe the standard (cheapest)
      wheel. If the 20-inch is standard, both records should become 300mi against ids 48745 /
      48755 — a 29-mile change to a headline figure. Rivian's own site would not render in the
      browser (heavy JS behind a cookie wall), so which wheel is standard is still unconfirmed.
      The existing sourced figure was kept rather than guessed; resolving it needs one fact.

## The remaining 7 are genuinely underivable, not unfinished

EPA has published nothing for these vehicles, and unlike the cases above there is no sibling close
enough to borrow from without asserting something false:

- **Cadillac Escalade IQ ×2** — no Escalade IQ in EPA's Cadillac list for 2025 or 2026 at all. The
  gas Escalade is a different vehicle.
- **Porsche Cayenne Electric ×2** — every 2026 Cayenne entry is the combustion car (`nullmi`).
  Matches the record's own `range: "Pending"`.
- **Tesla Model Y L** — absent from EPA's 2026 Tesla list. Deliberately *not* borrowed from the
  standard Model Y (Small SUV): the L is a longer three-row body and could well classify
  differently, which is exactly the kind of guess this field exists to avoid.
- **Rivian R2 (2027)** — not yet certified; a new, smaller platform with no sibling to borrow from.
