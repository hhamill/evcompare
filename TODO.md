...
# TODO: Research CCS1 entries for NACS/Supercharger adapter availability

Idea floated (2026-08-21), not started: for the 91 of 149 entries with `charging.portType: "CCS1"`, the data doesn't currently capture whether the manufacturer offers a NACS adapter enabling Tesla Supercharger access — several `notes` already mention this ad hoc for individual entries (e.g. Ford's ~$200 NACS-to-CCS1 adapter, Genesis's complimentary NACS adapter for the GV60, GMC's NACS-adapter-accessory for the Q8 e-tron) but it's not a structured, filterable field, so there's no way to compare across the whole dataset on this axis.

Would need a new field (something like `charging.nacsAdapterAvailable: true | false | "included" | null`, TBD exact shape) plus a real research pass across all 91 CCS1 makes (Acura, Audi, BMW, Cadillac, Chevrolet, Dodge, Fiat, Ford, Genesis, Honda, Hyundai, Jeep, Lucid, MINI, Mercedes-Benz, Nissan, Polestar, Porsche, Subaru, VinFast, Volkswagen, Volvo, Mercedes-Maybach) confirming whether each currently sells/bundles a NACS adapter. Not started — logged here to revisit as its own batch, same pattern as the other research-batch TODOs above.
