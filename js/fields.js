...
const BODY_STYLES = new Set(["Sedan", "Coupe", "Hatchback", "Crossover", "SUV", "Truck", "Minivan", "Van"]);

// Falls back to the crossover, not a lightning bolt: an unmapped body style should degrade to
// a generic vehicle rather than to something that reads as an error. That bolt was reachable
// in practice — "Van" was simply missing from the old emoji map.
export function bodyIcon(style) {
  const key = BODY_STYLES.has(style) ? style : "Crossover";
